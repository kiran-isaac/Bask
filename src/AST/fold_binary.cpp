...
unique_ptr<ASTExpr> ASTExpr::fold_binary(ASTExpr *expr) {
  ...
  if (!lhs || !rhs || lhs->type.kind != BASK_PRIMITIVE_TYPEKIND ||
      rhs->type.kind != BASK_PRIMITIVE_TYPEKIND) {
  ...
  if (lhs->type.primitive == BASK_STRING_PRIMITIVE || rhs->type.primitive == BASK_STRING_PRIMITIVE) {
    // v1 is definitely a string
    auto v1 = lhs->type.primitive == BASK_STRING_PRIMITIVE ? lhs : rhs;
    string s1 = v1->value;

    // v2 could be a string, a char
    auto v2 = lhs->type.primitive == BASK_STRING_PRIMITIVE ? rhs : lhs;
    string s2;
    ...
    switch (binary->op) {
      ...
      case BASK_TT_Operator_Equal:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, s1 == s2 ? "true" : "false", v1->line, v1->col);
  ...
  // If either is a float, then the other must be a float or an int
  } else if (lhs->type.primitive == BASK_FLOAT_PRIMITIVE ||
             rhs->type.primitive == BASK_FLOAT_PRIMITIVE) {
    // v1 is definitely a float
    auto v1 = lhs->type.primitive == BASK_FLOAT_PRIMITIVE ? lhs : rhs;

    // v2 could be a float, an int
    auto v2 = lhs->type.primitive == BASK_FLOAT_PRIMITIVE ? rhs : lhs;
    ...
    switch (binary->op) {
      case BASK_TT_Operator_Add:
        return make_unique<ASTExprConstantValue>(
            BASK_FLOAT_CONST, to_string(f1 + f2), v1->line, v1->col);
      ...
      case BASK_TT_Operator_Mod:
        return make_unique<ASTExprConstantValue>(
            BASK_FLOAT_CONST, to_string(fmod(f1, f2)), v1->line, v1->col);
      case BASK_TT_Operator_Equal:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, f1 == f2 ? "true" : "false", v1->line, v1->col);
      case BASK_TT_Operator_NotEqual:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, f1 != f2 ? "true" : "false", v1->line, v1->col);
  ...
  // If either is an int, then the other must be an int or a char (float has been handled above)
  } else if (lhs->type.primitive == BASK_INT_PRIMITIVE || rhs->type.primitive == BASK_INT_PRIMITIVE) {
    // v1 is definitely an int
    auto v1 = (lhs->type.primitive == BASK_INT_PRIMITIVE) ? lhs : rhs;

    auto v2 = (lhs->type.primitive == BASK_INT_PRIMITIVE) ? rhs : lhs;
    ...
    switch (binary->op) {
      case BASK_TT_Operator_Add:
        return make_unique<ASTExprConstantValue>(BASK_INT_CONST, to_string(i1 + i2),
                                                 v1->line, v1->col);
      case BASK_TT_Operator_Sub:
        return make_unique<ASTExprConstantValue>(BASK_INT_CONST, to_string(i1 - i2),
                                                 v1->line, v1->col);
      ...
      case BASK_TT_Operator_Equal:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, i1 == i2 ? "true" : "false", v1->line, v1->col);
      case BASK_TT_Operator_NotEqual:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, i1 != i2 ? "true" : "false", v1->line, v1->col);
      case BASK_TT_Operator_Less:
  ...
  // If either is a bool, then the other must be a bool
  } else if (lhs->type.primitive == BASK_BOOL_PRIMITIVE && rhs->type.primitive == BASK_BOOL_PRIMITIVE) {
    ...
    switch (binary->op) {
      case BASK_TT_Operator_Equal:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, b1 == b2 ? "true" : "false", lhs->line, lhs->col);
      case BASK_TT_Operator_NotEqual:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, b1 != b2 ? "true" : "false", lhs->line, lhs->col);
  ...
  // If either is a char, then the other must be a char (string and int are covered)
  } else if (lhs->type.primitive == BASK_CHAR_PRIMITIVE && rhs->type.primitive == BASK_CHAR_PRIMITIVE) {
    char c1 = lhs->value[0];
    char c2 = rhs->value[0];
    ...
    switch (binary->op) {
      case BASK_TT_Operator_Equal:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, c1 == c2 ? "true" : "false", lhs->line, lhs->col);
      case BASK_TT_Operator_NotEqual:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, c1 != c2 ? "true" : "false", lhs->line, lhs->col);
      case BASK_TT_Operator_Less:
      ...
      case BASK_TT_Operator_GreaterEqual:
        return make_unique<ASTExprConstantValue>(
            BASK_BOOL_CONST, c1 >= c2 ? "true" : "false", lhs->line, lhs->col);
      case BASK_TT_Operator_BitwiseAnd:
        return make_unique<ASTExprConstantValue>(BASK_CHAR_CONST, string(1, c1 & c2),
                                                 lhs->line, lhs->col);
      case BASK_TT_Operator_BitwiseOr:
        return make_unique<ASTExprConstantValue>(BASK_CHAR_CONST, string(1, c1 | c2),
                                                 lhs->line, lhs->col);
